    ...
    if ($r->have_posts()) :
        ...
        <ul<?php echo $ul_class; ?>>
            <?php while ($r->have_posts()) : $r->the_post(); ?>
                ...
                ?>
                <li<?php echo $li_class; ?>><a href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>"><?php if (get_the_title()) the_title();
            else the_ID();
            echo $count; ?></a></li>
        <?php endwhile; ?>
        </ul>
...
function bawpvc_settings_page() {
    ...
    function bawpvc_add_meta_boxes() {
        ...
        foreach ($cpts as $cpt)
            add_meta_box(
                    'bawpvc_meta_box', BAWPVC_FULLNAME, 'bawpvc_add_metabox', $cpt, 'side'
            );
    }
                ...
                foreach ($timings as $time => $date):
                    ...
                    if (current_user_can($capa, $post->ID))
                        printf('<li>' . __(
                                        ...
                                        '</span> views', 'bawpvc' .
                                        '</li>', 'bawpvc'), esc_html($time), (int) $count, $date, __('Clic to edit me!', 'bawpvc')
                        );
                    // else
