...
function getfinancialListings($numberOfListings) { //html
	...
	if($listings->found_posts > 0) {
		echo '<div class="container">
				<div id="sliderFeatureProduct" class="owl-carousel owl-theme">';
			while ($listings->have_posts()) {
				...
				$listItem = '<div class="item">
						<div class="financial-product-item">
							...
							<h5>'.get_the_title().'</h5>
							<p>'.get_the_excerpt().'</p>
							<a href="'.get_permalink().'" class="learn-more-btn"> Learn more <i class="glyphicon glyphicon-play"></i></a>
						</div>
					</div>';
...
function form($instance) {
	...
	$this->gettestimonialListings($numberOfListings);
	echo $after_widget;
}
...
function gettestimonialListings($numberOfListings) { //html
	global $post;
	//add_image_size( 'financial_widget_size', 85, 45, false );
	$listings = new WP_Query();
	$listings->query('post_type=testimonial&posts_per_page=' . $numberOfListings );
	if($listings->found_posts > 0) {
		echo '<section id="testimonial">			
			...
			<div class="container-fluid">
				<div id="sliderTestimonial" class="owl-carousel owl-theme">';
			while ($listings->have_posts()) {
				$listings->the_post();
				if (has_post_thumbnail( $post->ID ) ): 
					$image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'single-post-thumbnail' ); 
