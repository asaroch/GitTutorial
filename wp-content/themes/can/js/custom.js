		    ...
		    if ($window.scrollTop() >= main_navigation_height &&  isFixedMainNav != true) {
		     ...
		     if ( isHomePage )  {
	        	...
	        	if ($window.scrollTop() >= isSlider   && isGetQuoteFixed != true ) {
	       			$("#get_quote_home").removeClass('navbar-fixed-top').addClass('navbar-fixed-top').css({"position":"fixed","top":"54px"});
	        		console.log("home 1");
	        	} else if ($window.scrollTop() < isSlider && isGetQuoteFixed == true) {
	        		 $("#get_quote_home").removeClass('navbar-fixed-top').css({"position":"relative","top":"0px"});
	       			console.log("home 2");
	        	}
	        }
